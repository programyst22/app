...
import { BeforeAfter, MediaGrid } from "@/src/components/project-sections";
...
export default function MediaTab() {
  ...
  if (user === undefined || (client && isLoading)) return <Loading text="Medien werden geladen…" />;
  ...
  return (
    <ScrollView style={s.screen} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: insets.top + 26, paddingHorizontal: width < 700 ? 16 : 30, paddingBottom: 118 }}>
      <View style={[s.shell, { gap: 20 }]}>
        <View>
          <Text style={s.eyebrow}>BAUSTELLE VISUELL</Text>
          <Text style={[s.title, { fontSize: width < 700 ? 38 : 50, lineHeight: width < 700 ? 41 : 53, marginTop: 7 }]}>Fortschritt,{"\n"}den man sieht.</Text>
        </View>
        {!p ? (
          <View style={s.empty}><Text style={{ fontFamily: fonts.medium, fontSize: 16 }}>Noch kein aktives Projekt.</Text></View>
        ) : (
          <>
            <MediaGrid projectId={p.id} />
            <Text style={s.eyebrow}>VORHER / NACHHER</Text>
            <BeforeAfter projectId={p.id} />
          </>
        )}
      </View>
    </ScrollView>
